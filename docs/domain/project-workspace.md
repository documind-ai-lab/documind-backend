...
이 값은 도메인 엔티티에 하드코딩하지 않는다. 애플리케이션 설정이나 환경 변수에서 읽어 유스케이스 계층으로 주입하고, `Project` 생성 시에는 이미 결정된 `ownerId` 값을 전달한다. 운영 환경에서는 기본 사용자 ID 설정을 사용하지 않는다. 인증 기능 전 단계의 스테이징/데모 배포가 필요하면 `demo` 프로필에서만 `DOCUMIND_DEMO_OWNER_ID`를 명시적으로 주입한다. 운영 프로필에서 인증 기반 owner provider가 없으면 프로젝트 생성 유스케이스를 실행하지 않는다. API 계층의 오류 매핑은 원인별로 분리한다. 인증 정보가 없으면 401, 권한이 없으면 403, 서버 설정상 owner provider가 구성되지 않았으면 서버 설정 오류로 처리한다.

1차 MVP에서는 User/Auth Bounded Context가 아직 구현되지 않았으므로 `ownerId`는 외부 컨텍스트 식별자를 값으로 보관하는 참조다. Project 저장소는 사용자 테이블에 대한 DB Foreign Key를 강제하지 않는다. 후속 인증 컨텍스트가 도입되고 같은 데이터베이스 안에서 사용자 테이블을 관리하기로 결정되면, 마이그레이션이나 시드 데이터로 참조 무결성 보장 방식을 별도 설계한다.
...
API의 날짜/시간 필드는 RFC 3339 형식의 UTC 문자열로 직렬화한다. 예시는 `2026-06-29T01:00:00Z` 형식을 사용한다. 화면에서는 클라이언트가 한국 시간 등 사용자 표시 형식으로 변환한다. 저장소 구현에서는 타임존 정보를 보존하는 일시 타입을 사용한다.
...
기본 정렬은 `lastActivityAt DESC, id DESC`이다. 최근 활동한 프로젝트가 가장 위에 오고, 마지막 활동 시각이 같으면 UUID v7 기반 `id`가 타이브레이커 역할을 한다. 실제 저장소 구현에서는 `status`, `lastActivityAt`, `id` 기준 조회와 정렬을 고려한 복합 인덱스를 검토한다.

`lastActivityAt`은 사용자가 프로젝트 업무 맥락을 진전시키는 행동이 성공했을 때 갱신한다. 1차 MVP에서는 프로젝트 생성 시 `now`로 초기화하고, 프로젝트 기본 정보 수정, 보관, 복원만으로는 갱신하지 않는다. 생성 직후 빈 워크스페이스도 사용자가 방금 만든 작업 단위이므로 목록 상단에 노출되도록 `null`이 아니라 `now`로 초기화한다. 1차 MVP에서는 문서 업로드나 채팅이 아직 없으므로 `lastActivityAt`은 실질적으로 생성 시각과 동일하게 유지된다. 이는 목록의 최근 활동을 프로젝트 메타데이터 변경이 아니라 실제 업무 진행 기준으로 보여주기 위한 의도된 정책이다. 후속 기능에서는 문서 업로드, AI 채팅, 분석 결과 생성, 결정사항 추가, 리스크 후보 추가가 성공했을 때 갱신한다.

목록 응답은 카드와 테이블 렌더링에 필요한 요약 정보만 담는다. 목록형 API는 공통 페이징 응답 구조인 `PageResponse<T>` 형태를 사용한다.
...
`PATCH` 요청은 JSON Merge Patch 의미론을 따른다. 요청 본문에 없는 필드는 변경하지 않고, 명시적으로 `null`이 전달된 nullable 필드는 값을 비운다. 요청 DTO 또는 매퍼는 원본 JSON 객체의 키 존재 여부를 기준으로 필드 누락과 명시적 `null` 전달을 구분해야 한다. 이 요구사항은 특정 프레임워크 기능에 의존하지 않는 API 계약이다.
